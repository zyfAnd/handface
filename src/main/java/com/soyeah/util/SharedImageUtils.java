...
public class SharedImageUtils {
    ...
    // 要放置的二维码高度
    public static final int QRCODE_Y = 540;
    // 要放置的头像半径
    public static final int AVATAR_SIZE = 160;
    // 要放置的头像y坐标
    public static final int AVATAR_Y = 150;
    // 昵称的高度
    public static final int FONT_Y = 370;
    // 推广文案的高度
    public static final int COPYWRITER = 500;
    // 二维码识别图案高度
    public static final int RECOGNITION_QRCODE_Y = 890;
    // 二维码识别图案大小
    public static final int RECOGNITION_QRCODE_SIZE = 260;
    ...
    public static BufferedImage mergeQrcode(BufferedImage baseImage, String qrcodeImage, String topImage) throws IOException {
        BufferedImage recogBufferImage = null;
        BufferedImage qrcodeBufferImage = null;
        if(topImage!=null&&!topImage.equals("")){
            recogBufferImage =  ImageIO.read(new File(topImage));
        }

        if(qrcodeImage!=null&&qrcodeImage.length()>0){
            qrcodeBufferImage = ImageIO.read(new File(qrcodeImage));
        }
        ...
        if(qrcodeImage!=null&&!qrcodeImage.equals("")){
     ...
     * @throws Exception the exception
     */
    public static BufferedImage drawTextInImage(BufferedImage src, String copywriter,int textHeight,int tmpWidth) throws IOException {
        ...
        if (copywriter.getBytes().length > 63) {
            ...
            g.drawString(firstLine, (width - fm.stringWidth(firstLine)) / 2, textHeight);
            System.out.print("width-"+(width - fm.stringWidth(firstLine)) / 2);
            g.drawString(secondLine, (width - fm.stringWidth(secondLine)) / 2, textHeight + 35);
        } else {
//            int tmpWidth = (width - textWidth) / 2;
            g.drawString(copywriter, tmpWidth, textHeight);
        }
        g.dispose();
